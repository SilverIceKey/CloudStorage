...
public class FileServiceImpl extends ServiceImpl<FileInfoMapper, FileInfo> implements IFileService {
    ...
    public RestResponse uploadFile(FileUploadVo fileUploadVo) throws IOException {
        QueryWrapper<FileInfo> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("filemd5", fileUploadVo.getFileMD5().toUpperCase());
        FileInfo fileInfo = getBaseMapper().selectOne(queryWrapper);
        if (fileInfo != null) {
            return RestUtil.ok("极速上传成功", fileInfo);
        } else {
            OutputStream os = FileUtil.getOutputStream(Constants.FILE_SAVE_PATH + fileUploadVo.getMultipartFile().getOriginalFilename());
            ...
            fileInfo.setFilePath(Constants.FILE_SAVE_PATH + fileUploadVo.getMultipartFile().getOriginalFilename());
